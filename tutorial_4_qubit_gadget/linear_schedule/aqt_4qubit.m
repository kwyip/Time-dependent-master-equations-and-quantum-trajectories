...
parfor n = 1:ntraj
    ...
    for index = 1:numel(tstep_qt)
        ...
        if unpsicb'*unpsicb > r1
            ...
            Hs = -1e9.*(1-t_guess./tf).*(sX_1+sX_2+sX_3+sX_4) + 1e9.*t_guess.*tf.^(-1).*(((-1).*sZ_1+(0.95).*sZ_2+(0.95).*sZ_3+(-1).*sZ_4) + ...
                ((-1).*sZsZII + (-1).*IsZsZI + (-1).*IIsZsZ));
            [V,D] = eig(full(Hs));
            %[V,D] = eigs(Hs,18,'sa');
            if ~issorted(diag(D))
...
print -dpdf qt4qubitexample1_linear
...
ylabel('Ground State Population')
title(['tf: ' num2str(tf) ', Number of trajectories: ' num2str(ntraj)])
print -dpdf qt4qubitexample2_linear
...
txt1 = sprintf('qt4qubitexample_linear_fidelity_tf%d_ntraj%d.txt',tf,ntraj);
fid1 = fopen(txt1,'w');
...
fid3 = fopen('qt4qubitexample_jlistmat_cb.txt','w');
% fprintf(fid3,'%d\n',jlistmat);
% fclose(fid3);
