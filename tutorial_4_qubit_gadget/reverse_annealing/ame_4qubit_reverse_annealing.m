%kawayip@usc.edu
%8-qubit_chain
function ame_4qubit_reverse_annealing
...
p%si0 = sparse(kron(kron(kron(zero0,zero0),zero0),zero0));
rho0 = psi0*psi0';
%rho = rho0(:);
rho = rho0;
...
fouthexcitedpoplist_me = zeros(1, numel(tstep_me));
...
for index = 1:numel(tstep_me)
    ...
    fouthexcitedpoplist_me(1, index) = fouthexcitedpop;
...
hold on
plot(tstep_me, firstexcitedpoplist_me,'-','color', [0 0.5 0], 'LineWidth',2);
hold on
plot(tstep_me, secondexcitedpoplist_me,'-','color',[0.8500, 0.3250, 0.0980],'LineWidth',2);
hold on
plot(tstep_me, thirdexcitedpoplist_me,'-','color',[0.9290, 0.6940, 0.1250], 'LineWidth',2);
hold on
plot(tstep_me, fouthexcitedpoplist_me,'-', 'color',[0.50, 0.1850, 0.5580], 'LineWidth',2);
xlabel('$t$','Interpreter','latex','FontSize',25)
ylabel('Eigenstate population','FontSize',25)
xlim([0 tf])
ylim([0 1])
set(gca,'FontSize',20)
% legend('0thpop','1stpop','2ndpop','3rdpop','4thpop', 'location', 'best')
legend('0thpop','1stpop','2ndpop','3rdpop','4thpop', 'location', 'west')
title([ '$t_{f}=$ ' num2str(tf)],'Interpreter','latex','FontSize', 17)
print -dpdf ame_4qubit_reverse_annealing_1

figure(2)
plot(tstep_me./tf, fidelitylist_me,'-b','LineWidth',2);
hold on
plot(tstep_me./tf, firstexcitedpoplist_me,'-','color', [0 0.5 0], 'LineWidth',2);
hold on
plot(tstep_me./tf, secondexcitedpoplist_me,'-','color',[0.8500, 0.3250, 0.0980],'LineWidth',2);
hold on
plot(tstep_me./tf, thirdexcitedpoplist_me,'-','color',[0.9290, 0.6940, 0.1250], 'LineWidth',2);
hold on
plot(tstep_me./tf, fouthexcitedpoplist_me,'-', 'color',[0.50, 0.1850, 0.5580], 'LineWidth',2);
...
legend('0thpop','1stpop','2ndpop','3rdpop','4thpop', 'location', 'west')
title([ '$t_{f}=$ ' num2str(tf)],'Interpreter','latex','FontSize', 17)
print -dpdf ame_4qubit_reverse_annealing_2
...
txt1 = sprintf('ame_reverse%d.txt',tf);
fid1 = fopen(txt1,'w');
fprintf(fid1,'%13d %8d %8d %8d %8d %8d\n',[tstep_me;fidelitylist_me;firstexcitedpoplist_me;secondexcitedpoplist_me;thirdexcitedpoplist_me;fouthexcitedpoplist_me]);
fclose(fid1);
